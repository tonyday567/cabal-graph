...
tarEntries :: IO [Tar.Entry]
tarEntries = entryList . Tar.read <$> (BSL.readFile =<< cabalIndex)
  where
...
entryCounts xs =
  ...
  where
    ...
    xs'' = xs' & filter (isPackageJson >>> not)
    xs''' = xs'' & Set.fromList & Set.toList
    allentries = length xs
    pvs = length xs - length xs'
    pjs = length xs' - length xs''
    overwritten = length xs'' - length xs'''
    unique = length xs'''

-- | List of most-recent, unique-versioned cabal files
--
-- Ignores:
--
-- - stale versions with later revisions
...
-- latest valid one.
latestCabals :: [(FileName, BSL.ByteString)] -> Map.Map ByteString (Version, ByteString)
latestCabals cs =
  Map.fromListWith (\new old -> bool old new (fst new >= fst old)) $ (\(fn, bs) -> (nameFN fn, (getVersion fn, bs))) . second B.toStrict <$> cs
  where
    getVersion = fromMaybe (error "bad version") . simpleParsecBS . versionFN

-- | Latest successfully parsing 'CabalFields'
latestCabalFields :: Config -> Map.Map ByteString (Version, ByteString) -> Map.Map ByteString (Version, CabalFields)
latestCabalFields cfg lcs = second (fromRight (error "bad field parse")) <$> Map.filter (snd >>> isRight) lcs'
  where
    lcs' = second (parseCabalFields cfg) <$> lcs
